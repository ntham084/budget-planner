...
import { getGoalProgress } from "@/lib/finance";
...
function makeEmptyForm(defaultAccountId: string): GoalFormValues {
  return {
    name: "",
    targetAmount: "",
    currentAmount: "0",
    targetDate: "",
    linkedAccountId: defaultAccountId,
  };
}
...
export default function GoalsPage() {
  ...
  const [form, setForm] = useState<GoalFormValues>(makeEmptyForm(""));
  ...
  function openAddModal() {
    setEditingId(null);
    setForm(makeEmptyForm(accounts[0]?.id ?? ""));
    setModalOpen(true);
  }
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    if (!payload.name || !payload.linkedAccountId) return;
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-slate-400">
                        Linked Account: {accountName(goal.linkedAccountId)}
                      </p>
                    </div>
