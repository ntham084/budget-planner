...
import { amountForPercent, spentInCategory } from "@/lib/selectors";
...
export default function BudgetPage() {
  ...
  const spendingCategories = data.paycheck.categories.filter(
    (c) => c.isSpendingBudget,
  );

  const rows = spendingCategories.map((category) => {
    const budgeted = amountForPercent(category.percent, paycheckAmount);
    const spent = spentInCategory(data.transactions, category.name);
